...
window.finishWeek = async function () {
  ...
  const weekProducts = products.filter(
    p => p.week === info.week && p.month === info.month && p.year === info.year
  );

  const totalPrice    = weekProducts.reduce((s, p) => s + Number(p.price || 0), 0);
  const totalCalories = weekProducts.reduce((s, p) => s + Number(p.calories || 0), 0);
  ...
  let nextWeek  = info.week + 1;
  let nextMonth = info.month;
  ...
  alert(`✅ Тиждень ${info.week} завершено!\nПереходимо на Тиждень ${nextWeek}.`);
};
